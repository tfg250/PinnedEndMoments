...
def WriteInput(INPUTS):
    ...
    for d in INPUTS['Diaphragm']:
        ...
        if type(d[3])==float:
            ...
            inpfile.write('*DIAPHRAGM,'+str(dno)+','+str(d[3][0])+','+str(d[3][1])+','+str(
                d[3][2])+','+str(d[1])+','+str(d[2])+','+str(d[0]*d1L)+',')
            inpfile.write(','.join([str(j) for j in INPUTS['Diaphragm_Common']])+'\n')
...
if True:
    ...
    Girder_I = [0, (1, 0), 1.0]
    # Girder_common = [MaterialNo, DL0-NSM, DL1-NSM, DL2-NSM, SIDL-NSM, COMP, Stiffness Factor, Top Flange Unbraced Length, Bottom Flange Unbraced Length, transverse stiffener spacing, COPE]
    Girder_Icommon = [1, 0, 0, 0, 0, 'C', 1, 1, 0, 625/3., 625/3., 0]
    Girder_Ecommon = [1, 0, 0, 0, 0, 'C', 1, 1, 0, 625/3., 625/3., 0]
    GirderType = 'C'
    #           [x,mat,prop,z]
    #A,B,C,A
    Diaphragm = [[0, 1, 4, gz+10.5], [0.33, 1, 5, (gz+17.5,'x',gz-17.5)], [0.33, 1, 5, (gz-17.5,'x',gz+17.5)], [0.33, 1, 5, gz-17.5], [0.67, 1, 6, gz+13.5], [0.67, 1, 7, (gz-17.5,gz+13.5,gz-17.5)], [0.67, 1, 5, gz-17.5], [1.0, 1, 4, gz+10.5]]
    Diaphragm_Common = ['straight', 0, 0, 0, 0, 1]
    Props = {}
    Props[1] = [0, 'AISC','13th','W36x135']
    #Props[2] = [0, 'IBeam', 16,16,54.5,1.625,0.875,0.375]
    Props[4] = [0, 'AISC', '13th','W14x30']
    Props[5] = [0, 'Angle', 5,3.5,0.375,0.375]
    Props[6] = [0, 'AISC', '13th','WT8X28.5']
    Props[7] = [0, 'Angle', 3,3,0.375,0.375]
...
if True:
    ...
    INPUTS['000']['Label'] = "Superstructure Only"

    INPUTS["%03d" % 1] = copy.deepcopy(INPUTS['000'])
    INPUTS["%03d" % 1]['ModelName'] = ModelName+"%03d" % 1
    INPUTS["%03d" % 1]['Label'] = "Including Substructure"
# -------------------------------------------------------------------------------
# Parameterize
# -------------------------------------------------------------------------------
if False:
...
#'001' exists, made with a manual mod to add floobeam and substructure elements
...
if False:
    MomentPlot(MODELDATA, INPUTS, run1[1:], 'ParametricController_MomentSummary5', norm=True)
    DeflectionPlot(MODELDATA, INPUTS, run1[1:], 'ParametricController_DeflectionSummary5')

    MomentPlot(MODELDATA, INPUTS, run2, 'ParametricController_MomentSummary6', norm=True)
    DeflectionPlot(MODELDATA, INPUTS, run2, 'ParametricController_DeflectionSummary6')
...
if True:
    MomentPlot(MODELDATA, INPUTS, run, 'ParametricController_MomentSummary7','quasi', norm=True)
